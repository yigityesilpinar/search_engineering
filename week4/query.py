...
def get_opensearch(the_host: str):
    auth = ('admin', 'admin')
    client = OpenSearch(
        hosts=[the_host],
...
@click.command()
@click.option('--query_file', '-q', default="/workspace/datasets/train.csv",
              help='Path to train.csv or test.csv or similar file containing queries')
@click.option('--index_name', '-i', default="bbuy_products", help="The name of the index to write to")
@click.option('--host', '-o', default="https://localhost:9200", help="The name of the host running OpenSearch")
...
@click.option('--seed', '-s', default=42,
              help="The seed to use for random sampling.  If multiple workers are used, a different seed will be given to each worker as a multiple of this seed.")
...
def main(query_file: str, index_name: str, host: str, max_queries: int, seed: int, workers: int):
    ...
    with Manager() as manager:
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=workers) as executor:
            futures = [executor.submit(query_opensearch, i, query_file, host, index_name, max_queries, seed * (i + 1),
                                       stop_event) for i in range(workers)]
